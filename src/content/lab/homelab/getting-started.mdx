...
This guide walks through creating a clean baseline system for a headless Raspberry Pi running Ubuntu Server 24.04 LTS.

The goal is not to deploy services yet, but to prepare a stable and secure foundation that future projects can build on — containers, VPNs, monitoring tools, self-hosted apps, or experiments.
...
- Firewall, automatic updates, and basic maintenance configured

Your values (hostname, username, network settings) can be different.
...
Replace the addresses below with your own trusted machines:

```bash
sudo ufw allow from 192.168.1.72 to any port 22 proto tcp
sudo ufw allow from 192.168.1.71 to any port 22 proto tcp
sudo ufw allow from 100.109.113.127 to any port 22 proto tcp
```

Restricting SSH access significantly reduces the attack surface.
...
Password-based logins are now disabled, preventing automated brute-force attempts.
